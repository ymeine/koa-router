  ...
  var dispatch = function *dispatch(next) {
    ...
    if (matched.pathAndMethod.length) {
      ...
      while (matched.route && i--) {
        ...
        var params = layer.params(path, this.captures, {});
        debug('dispatch %s %s %s', layer.path, layer.regexp, ii);

        var returned_value;
        while (ii--) {
          if (layer.stack[ii].constructor.name === 'GeneratorFunction') {
            debug('calling a generator function');
            next = layer.stack[ii].call(this, next, params);
            returned_value = next;
          } else {
            debug('calling a standard function');
            returned_value = layer.stack[ii].call(this, next, params);
            next = Promise.resolve(returned_value);
          }
          debug('new next value: %s (%s)', util.inspect(returned_value), returned_value);
        }
      }
    }
